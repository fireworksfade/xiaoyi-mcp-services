...
class StaticBearerTokenVerifier:
    ...
    async def verify_token(self, token: str) -> AccessToken | None:
        ...
        return AccessToken(
            token=token,
            client_id="iot-control-backend",
            scopes=["mcp:invoke"],
            subject="iot-control-client",
        )


def auth_configuration() -> tuple[AuthSettings | None, StaticBearerTokenVerifier | None]:
    token = os.getenv("CONTROL_MCP_BEARER_TOKEN", "").strip()
    if not token:
        return None, None
    public_url = os.getenv("CONTROL_MCP_PUBLIC_URL", "http://127.0.0.1:9002").rstrip("/")
